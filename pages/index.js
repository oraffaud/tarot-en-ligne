...
function freeCardReading(card, lang) {
  if (!card) return ''
  if (lang === 'en') return `${card.name} brings forward the energy of ${card.up.toLowerCase()}. It suggests that something meaningful is already moving around your question, but its full direction is not yet visible.`
...
export default function Home() {
  ...
  function openRitual() {
    if (!question.trim()) { setError(lang === 'en' ? 'Write the question you would like to entrust to the cards.' : 'Écrivez la question que vous souhaitez confier aux cartes.'); return }
    setError(''); setStep(2)
  }
  function startSpread() { setCards(drawCards(count)); setDrawnCount(0); setError(''); setStep(3) }
  function drawNextCard() { if (drawnCount < count) setDrawnCount(v => Math.min(v + 1, count)) }
  function restart() { setQuestion(''); setCount(3); setCards([]); setDrawnCount(0); setStep(1); setError('') }

  async function goPremium() {
    try {
      const context = { question: question.trim(), lang, count, cards: cards.map(c => ({ name: c.name, up: c.up, rev: c.rev, idx: c.idx })) }
      const r = await fetch('/api/stripe/create-checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ context }) })
    ...
    } catch (e) { setError(lang === 'en' ? 'Payment could not be opened. Please try again.' : 'Le paiement n’a pas pu être ouvert. Merci de réessayer.') }
  }

  return <div className="min-h-screen bg-gradient-to-b from-[#1a1022] via-[#2b1739] to-[#120b18] text-white">
    <Head><title>{lang === 'en' ? "Nanou’s Tarot — Free reading" : 'Les tarots de Nanou — Tirage découverte'}</title><meta name="description" content={lang === 'en' ? 'Ask your question, draw the Tarot de Marseille cards yourself and discover a first symbolic reading.' : 'Posez votre question, tirez vous-même les cartes du Tarot de Marseille et découvrez un premier éclairage symbolique.'}/><link rel="icon" href="/favicon.svg" /></Head>
    <Header />
    <main className="max-w-5xl mx-auto px-5 md:px-6 pb-20">
      ...
      {step === 3 && <section className="max-w-4xl mx-auto rounded-[28px] bg-white/[0.055] border border-white/10 p-7 md:p-10 shadow-2xl mt-12"><div className="text-center"><div className="text-xs uppercase tracking-[0.25em] text-amber-100/65">{lang==='en'?'The ritual begins':'Le rituel commence'}</div><h2 className="text-2xl md:text-4xl font-serif mt-3">{drawnCount<count?(lang==='en'?'Draw the cards yourself':'Tirez vous-même les cartes'):(lang==='en'?'Your spread is set':'Votre tirage est posé')}</h2></div>{drawnCount<count&&<div className="mt-9 flex flex-col items-center"><button onClick={drawNextCard} className="free-tarot-deck"><span className="free-deck-shadow free-deck-shadow-1"/><span className="free-deck-shadow free-deck-shadow-2"/><span className="free-deck-face"><span className="free-deck-moon">☾</span><span className="free-deck-star">✦</span><span className="free-deck-moon free-deck-moon-right">☽</span></span></button><div className="mt-5 text-sm uppercase tracking-[0.16em] text-amber-100/70">{lang==='en'?`Card ${drawnCount+1} of ${count}`:`Carte ${drawnCount+1} sur ${count}`}</div></div>}{drawnCount>0&&(count===5?<div className="tarot-cross mt-9 mx-auto">{cards.slice(0,drawnCount).map((c,i)=>{const positions=['cross-left','cross-right','cross-top','cross-bottom','cross-center'];return <div key={`${c.name}-${i}`} className={`cross-card ${positions[i]}`}><div className="cross-label">{i+1} · {labels[i]}</div><TarotCard name={c.name} meaning={{up:c.up,rev:c.rev}} index={c.idx}/></div>})}</div>:<div className={`mt-9 grid gap-6 ${count===1?'grid-cols-1 max-w-xs mx-auto':'grid-cols-3 max-w-3xl mx-auto'}`}>{cards.slice(0,drawnCount).map((c,i)=><div key={`${c.name}-${i}`} className="flex flex-col items-center"><div className="cross-label mb-2">{i+1} · {labels[i]}</div><TarotCard name={c.name} meaning={{up:c.up,rev:c.rev}} index={c.idx}/></div>)}</div>)}{drawnCount===count&&<div className="mt-10 text-center"><button onClick={()=>setStep(4)} className="bg-amber-200 hover:bg-amber-100 text-violet-950 px-8 py-3.5 rounded-full font-semibold shadow-lg transition">{lang==='en'?'Discover the first message ✦':'Découvrir le premier message ✦'}</button></div>}</section>}
      {step === 4 && <section className="max-w-4xl mx-auto mt-12"><div className="text-center"><div className="text-2xl text-amber-100">☾ ✦ ☽</div><div className="text-xs uppercase tracking-[0.25em] text-amber-100/65 mt-3">{lang==='en'?'Your first reading':'Votre premier éclairage'}</div><h2 className="text-3xl md:text-5xl font-serif mt-3">{lang==='en'?'The cards have begun to speak':'Les cartes ont commencé à parler'}</h2></div><div className="mt-8 space-y-4">{cards.map((c,i)=><article key={`${c.name}-${i}`} className="rounded-2xl bg-white/[0.055] border border-white/10 p-5 md:p-6"><div className="text-xs uppercase tracking-[0.18em] text-amber-100/65">{labels[i]}</div><h3 className="font-serif text-xl mt-1">{c.name}</h3><p className="mt-3 leading-8 text-violet-100">{freeCardReading(c,lang)}</p></article>)}</div><div className="mt-8 rounded-[28px] border border-amber-100/25 bg-gradient-to-b from-amber-100/[0.09] to-white/[0.03] p-7 md:p-9 text-center shadow-2xl"><div className="text-2xl">✦</div><h2 className="text-2xl md:text-4xl font-serif mt-3">{lang==='en'?'Your spread holds more than five separate messages':'Votre tirage contient bien plus que des cartes isolées'}</h2><p className="mt-4 max-w-2xl mx-auto text-violet-100 leading-8">{lang==='en'?'The full reading reveals what links the cards together, what remains hidden, where tensions appear and what direction the spread may be opening.':'La lecture complète révèle ce qui relie les cartes entre elles, ce qui reste encore dans l’ombre, où se situent les tensions et quelle direction le tirage semble ouvrir.'}</p><button onClick={goPremium} className="mt-8 bg-amber-200 hover:bg-amber-100 text-violet-950 px-9 py-4 rounded-full font-semibold shadow-lg transition text-lg">{lang==='en'?'Unlock my complete reading · €19 ✦':'Débloquer ma lecture complète · 19 € ✦'}</button><p className="mt-4 text-sm text-violet-400">{lang==='en'?'Secure payment by Stripe. Your question and spread are preserved.':'Paiement sécurisé par Stripe. Votre question et votre tirage sont conservés.'}</p></div><div className="mt-8 rounded-[28px] bg-white/[0.045] border border-white/10 p-6 md:p-8"><LiveChatPromo /></div><div className="mt-8 flex flex-wrap justify-center gap-4"><button onClick={restart} className="text-sm underline text-violet-300">{lang==='en'?'Begin another reading':'Commencer un autre tirage'}</button><a href="/jeu" className="text-sm underline text-violet-300">{lang==='en'?'View the Tarot de Marseille deck':'Voir le jeu du Tarot de Marseille'}</a></div></section>}
    </main>
    <style jsx global>{`
      .free-tarot-deck{position:relative;width:168px;height:258px;border:0;padding:0;background:transparent;cursor:pointer;filter:drop-shadow(0 22px 28px rgba(0,0,0,.42))}.free-deck-shadow,.free-deck-face{position:absolute;inset:0;border-radius:16px}.free-deck-shadow{background:#281538;border:1px solid rgba(255,240,205,.22)}.free-deck-shadow-1{transform:translate(9px,8px) rotate(2.2deg);opacity:.70}.free-deck-shadow-2{transform:translate(5px,4px) rotate(1deg);opacity:.86}.free-deck-face{display:flex;align-items:center;justify-content:center;color:#f0dca6;background:linear-gradient(145deg,#3b1d50,#1b1026);border:2px solid rgba(245,216,153,.58);box-shadow:inset 0 0 0 8px rgba(20,8,28,.62)}.free-deck-star{font-size:54px}.free-deck-moon{position:absolute;top:30px;left:24px;font-size:25px}.free-deck-moon-right{top:auto;left:auto;right:24px;bottom:30px}.tarot-cross{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));grid-template-rows:repeat(3,auto);align-items:center;justify-items:center;width:100%;max-width:760px;gap:10px}.cross-card{display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:0}.cross-left{grid-column:1;grid-row:2}.cross-right{grid-column:3;grid-row:2}.cross-top{grid-column:2;grid-row:1}.cross-bottom{grid-column:2;grid-row:3}.cross-center{grid-column:2;grid-row:2}.cross-label{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:rgba(245,230,255,.65);white-space:nowrap}
    `}</style>
  </div>
}
