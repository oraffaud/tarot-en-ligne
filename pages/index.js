...
const PAYMENT_LINK = process.env.NEXT_PUBLIC_PAYMENT_LINK_URL || '/premium'
...
function labels(count, lang) {
  ...
  if (count === 3) return lang === 'en' ? ['Past', 'Present', 'Future'] : ['Passé', 'Présent', 'Futur']
  return lang === 'en' ? ['Left', 'Right', 'Top', 'Bottom', 'Centre'] : ['Gauche', 'Droite', 'Haut', 'Bas', 'Centre']
}

function firstReading(card, lang) {
  if (!card) return ''
  return lang === 'en'
    ? `${card.name} brings forward ${card.up.toLowerCase()}. A meaningful movement is already forming around your question, but the whole message is not visible yet.`
    : `${card.name} fait émerger une énergie de ${card.up.toLowerCase()}. Quelque chose d’important est déjà en mouvement autour de votre question, mais l’ensemble du message ne se révèle pas encore.`
}
...
export default function Home() {
  ...
  const spreadLabels = labels(count, lang)

  const openRitual = () => {
    if (!question.trim()) {
      setError(lang === 'en' ? 'Write the question you would like to entrust to the cards.' : 'Écrivez la question que vous souhaitez confier aux cartes.')
      return
    }
    setError('')
    setStep(2)
  ...
  const goPremium = () => {
    const context = { question: question.trim(), lang, count, cards }
    try { window.localStorage.setItem('nanou_premium_context', JSON.stringify(context)) } catch {}
      ...
      <main className="max-w-5xl mx-auto px-5 md:px-6 pb-20">
        {step === 1 && (
          <section className="max-w-3xl mx-auto text-center pt-14 md:pt-20">
            ...
            <div className="text-xs uppercase tracking-[0.30em] text-amber-100/65">{lang === 'en' ? 'A first opening' : 'Un premier passage'}</div>
            <h1 className="text-4xl md:text-6xl font-serif mt-4 leading-tight">{lang === 'en' ? 'A question keeps returning?' : 'Une question revient sans cesse ?'}</h1>
            <p className="mt-5 max-w-2xl mx-auto text-violet-100/90 leading-8 text-lg">{lang === 'en' ? 'Take a quiet moment. Think of what you would truly like to understand, then entrust it to the Tarot de Marseille.' : 'Prenez un instant au calme. Pensez à ce que vous aimeriez réellement comprendre, puis confiez-le au Tarot de Marseille.'}</p>

            <div className="mt-10 rounded-[28px] bg-white/[0.055] border border-white/10 p-6 md:p-8 shadow-2xl text-left">
              <label className="block text-sm uppercase tracking-[0.18em] text-violet-300 mb-3">{lang === 'en' ? 'Your question' : 'Votre question'}</label>
              <textarea value={question} onChange={e => setQuestion(e.target.value)} rows={5} className="w-full rounded-2xl p-5 text-violet-950 min-h-[150px] shadow-inner text-lg" placeholder={lang === 'en' ? 'What would you like the cards to illuminate today?' : 'Qu’aimeriez-vous que les cartes viennent éclairer aujourd’hui ?'} />
              {error && <div className="mt-4 rounded-xl border border-red-200/20 bg-red-200/10 px-4 py-3 text-red-100">{error}</div>}
              <div className="mt-6 text-center"><button onClick={openRitual} className="bg-amber-200 hover:bg-amber-100 text-violet-950 px-8 py-3.5 rounded-full font-semibold shadow-lg transition">{lang === 'en' ? 'Open my reading ✦' : 'Ouvrir mon tirage ✦'}</button></div>
            </div>
            <p className="mt-6 text-sm text-violet-400">{lang === 'en' ? 'A first symbolic reading offered · no account required' : 'Un premier éclairage symbolique offert · sans création de compte'}</p>
          </section>
        )}

        {step === 2 && (
          <section className="max-w-3xl mx-auto rounded-[28px] bg-white/[0.055] border border-white/10 p-7 md:p-10 shadow-2xl mt-12">
            <div className="text-center">
              <div className="text-xs uppercase tracking-[0.25em] text-amber-100/65">{lang === 'en' ? 'The intention is set' : 'L’intention est posée'}</div>
              <h2 className="text-2xl md:text-4xl font-serif mt-3">{lang === 'en' ? 'Choose how the cards will answer' : 'Choisissez comment les cartes vont vous répondre'}</h2>
              <p className="mt-4 text-violet-200 leading-7">{lang === 'en' ? 'The deeper the spread, the more nuances appear.' : 'Plus le tirage est profond, plus les nuances apparaissent.'}</p>
            </div>
        ...
        {step === 4 && (
          <section className="max-w-4xl mx-auto mt-12">
            <div className="text-center"><div className="text-2xl text-amber-100">☾ ✦ ☽</div><div className="text-xs uppercase tracking-[0.25em] text-amber-100/65 mt-3">{lang==='en'?'Your first reading':'Votre premier éclairage'}</div><h2 className="text-3xl md:text-5xl font-serif mt-3">{lang==='en'?'The cards have begun to speak':'Les cartes ont commencé à parler'}</h2><p className="mt-4 max-w-2xl mx-auto text-violet-200 leading-8">{lang==='en'?'Each card reveals a fragment. The deeper meaning appears in the way they answer one another.':'Chaque carte révèle un fragment. Le sens le plus profond apparaît dans la manière dont elles se répondent.'}</p></div>
            <div className="mt-8 space-y-4">{cards.map((c,i)=><article key={`${c.name}-${i}`} className="rounded-2xl bg-white/[0.055] border border-white/10 p-5 md:p-6"><div className="text-xs uppercase tracking-[0.18em] text-amber-100/65">{spreadLabels[i]}</div><h3 className="font-serif text-xl mt-1">{c.name}</h3><p className="mt-3 leading-8 text-violet-100">{firstReading(c,lang)}</p></article>)}</div>

            <div className="mt-8 rounded-[28px] border border-amber-100/25 bg-gradient-to-b from-amber-100/[0.09] to-white/[0.03] p-7 md:p-9 text-center shadow-2xl">
              <div className="text-2xl">✦</div><h2 className="text-2xl md:text-4xl font-serif mt-3">{lang==='en'?'Your spread holds more than separate messages':'Votre tirage contient bien plus que des cartes isolées'}</h2><p className="mt-4 max-w-2xl mx-auto text-violet-100 leading-8">{lang==='en'?'The full reading reveals what links the cards together, what remains hidden and what direction the spread may be opening.':'La lecture complète révèle ce qui relie les cartes entre elles, ce qui reste encore dans l’ombre et quelle direction le tirage semble ouvrir.'}</p>
              <div className="grid md:grid-cols-3 gap-3 mt-7 text-left">{[lang==='en'?'The dialogue between the cards':'Le dialogue entre les cartes',lang==='en'?'What your question awakens':'Ce que votre question met en mouvement',lang==='en'?'The deeper symbolic message':'Le message symbolique profond'].map(label=><div key={label} className="rounded-2xl border border-white/10 bg-black/15 p-5"><div className="text-xl text-amber-100/70">✦</div><div className="mt-3 text-violet-100">{label}</div></div>)}</div>
              <button onClick={goPremium} className="mt-8 bg-amber-200 hover:bg-amber-100 text-violet-950 px-9 py-4 rounded-full font-semibold shadow-lg transition text-lg">{lang==='en'?'Unlock my complete reading · €19.90 ✦':'Débloquer ma lecture complète · 19,90 € ✦'}</button>
              <p className="mt-4 text-sm text-violet-400">{lang==='en'?'Secure payment. Your question and spread are preserved.':'Paiement sécurisé. Votre question et votre tirage sont conservés.'}</p>
            </div>

            <div className="mt-8 rounded-[28px] bg-white/[0.045] border border-white/10 p-7 md:p-8 text-center"><div className="text-xs uppercase tracking-[0.22em] text-violet-300">{lang==='en'?'And if a question remains…':'Et si une question demeure…'}</div><h3 className="text-2xl md:text-3xl font-serif mt-3">{lang==='en'?'Continue privately with Nanou':'Poursuivez en privé avec Nanou'}</h3><p className="mt-3 text-violet-200 leading-7 max-w-2xl mx-auto">{lang==='en'?'After the Premium reading, you can continue the exchange directly with Nanou in a private chat.':'Après la lecture Premium, vous pourrez prolonger l’échange directement avec Nanou dans un chat privé.'}</p></div>

            <div className="mt-8 text-center"><button onClick={restart} className="text-sm underline text-violet-300">{lang==='en'?'Begin another reading':'Commencer un autre tirage'}</button></div>
          </section>
        )}
      </main>
