    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1a1022] via-[#2b1739] to-[#120b18] text-white">
      <Head>
        ...
        <meta
          name="description"
          content={lang === 'en'
            ? 'Ask your question, draw the Tarot de Marseille cards yourself and discover a first symbolic reading.'
            : 'Posez votre question, tirez vous-même les cartes du Tarot de Marseille et découvrez un premier éclairage symbolique.'}
        />
      ...
      <main className="max-w-5xl mx-auto px-5 md:px-6 pb-20">
        {step === 1 && (
          <section className="max-w-3xl mx-auto text-center pt-14 md:pt-20">
            ...
            <div className="text-xs uppercase tracking-[0.30em] text-amber-100/65">
              {lang === 'en' ? 'A first opening' : 'Un premier passage'}
            </div>
            ...
            <p className="mt-5 max-w-2xl mx-auto text-violet-100/90 leading-8 text-lg">
              {lang === 'en'
                ? 'Take a quiet moment. Think of what you would truly like to understand, then entrust it to the Tarot de Marseille.'
                : 'Prenez un instant au calme. Pensez à ce que vous aimeriez réellement comprendre, puis confiez-le au Tarot de Marseille.'}
            </p>
            ...
            <div className="mt-10 rounded-[28px] bg-white/[0.055] border border-white/10 p-6 md:p-8 shadow-2xl text-left">
              ...
              <textarea
                ...
                placeholder={lang === 'en'
                  ? 'What would you like the cards to illuminate today?'
                  : 'Qu’aimeriez-vous que les cartes viennent éclairer aujourd’hui ?'}
              />
              ...
              <div className="mt-6 text-center">
                ...
                >
                  {lang === 'en' ? 'Open my reading ✦' : 'Ouvrir mon tirage ✦'}
                </button>
              </div>
            </div>
            ...
            <p className="mt-6 text-sm text-violet-400">
              {lang === 'en'
                ? 'A first symbolic reading offered · no account required'
                : 'Un premier éclairage symbolique offert · sans création de compte'}
            </p>
          </section>
        )}
        ...
        {step === 2 && (
          <section className="max-w-3xl mx-auto rounded-[28px] bg-white/[0.055] border border-white/10 p-7 md:p-10 shadow-2xl mt-12">
            <div className="text-center">
              ...
              <h2 className="text-2xl md:text-4xl font-serif mt-3">
                {lang === 'en' ? 'Choose how the cards will answer' : 'Choisissez comment les cartes vont vous répondre'}
              </h2>

              <p className="mt-4 text-violet-200 leading-7">
                {lang === 'en'
                  ? 'The deeper the spread, the more nuances appear.'
                  : 'Plus le tirage est profond, plus les nuances apparaissent.'}
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4 mt-8">
              {[
                {
                  id: 1,
                  title: lang === 'en' ? 'The sign' : 'Le signe',
                  desc: lang === 'en' ? '1 card · a direct message' : '1 carte · un message direct'
                },
                {
                  id: 3,
                  title: lang === 'en' ? 'The path' : 'Le chemin',
                  desc: lang === 'en' ? '3 cards · past, present, future' : '3 cartes · passé, présent, futur'
                },
                {
                  id: 5,
                  title: lang === 'en' ? 'The cross' : 'La croix',
                  desc: lang === 'en' ? '5 cards · a deeper symbolic map' : '5 cartes · une lecture plus profonde'
                }
              ].map(o => (
                <button
                  key={o.id}
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <button
                ...
                className="bg-amber-200 hover:bg-amber-100 text-violet-950 px-8 py-3.5 rounded-full font-semibold shadow-lg transition"
              >
                {lang === 'en' ? 'Begin the ritual ✦' : 'Commencer le rituel ✦'}
              </button>
          ...
          <section className="max-w-4xl mx-auto rounded-[28px] bg-white/[0.055] border border-white/10 p-7 md:p-10 shadow-2xl mt-12">
            <div className="text-center">
              ...
              <p className="mt-4 text-violet-200 leading-7 max-w-2xl mx-auto">
                {drawnCount < count
                  ? (lang === 'en'
                      ? 'Keep your question in mind. Touch the deck each time you feel ready.'
                      : 'Gardez votre question à l’esprit. Touchez le jeu chaque fois que vous vous sentez prêt.')
                  : (lang === 'en'
                      ? 'The cards have taken their place. Observe them before discovering the first message.'
                      : 'Les cartes ont trouvé leur place. Observez-les avant de découvrir leur premier message.')}
              </p>
            </div>
                ...
                {count === 5 ? (
                  <div className="tarot-cross mt-9 mx-auto">
                    {cards.slice(0, drawnCount).map((c, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="cross-label">{i + 1} · {labels[i]}</div>
                  ...
                  <div className={`mt-9 grid gap-6 ${count === 1 ? 'grid-cols-1 max-w-xs mx-auto' : 'grid-cols-3 max-w-3xl mx-auto'}`}>
                    {cards.slice(0, drawnCount).map((c, i) => (
                      <div
                        key={`${c.name}-${i}`}
                        className="flex flex-col items-center animate-[cardReveal_.75s_cubic-bezier(.2,.8,.2,1)]"
                        style={{ animationFillMode: 'both' }}
                      >
                        <div className="cross-label mb-2">{i + 1} · {labels[i]}</div>
                        <TarotCard name={c.name} meaning={{ up: c.up, rev: c.rev }} index={c.idx} />
                      </div>
                    ))}
                  </div>
            ...
            {drawnCount === count && (
              <div className="mt-10 text-center">
                <button
                  ...
                  className="bg-amber-200 hover:bg-amber-100 text-violet-950 px-8 py-3.5 rounded-full font-semibold shadow-lg transition"
                >
                  {lang === 'en' ? 'Discover the first message ✦' : 'Découvrir le premier message ✦'}
                </button>
              </div>
            )}
          </section>
        ...
        {step === 4 && (
          <section className="max-w-4xl mx-auto mt-12">
            <div className="text-center">
              <div className="text-2xl text-amber-100">☾ ✦ ☽</div>
              <div className="text-xs uppercase tracking-[0.25em] text-amber-100/65 mt-3">
                {lang === 'en' ? 'Your first reading' : 'Votre premier éclairage'}
              </div>
              <h2 className="text-3xl md:text-5xl font-serif mt-3">
                {lang === 'en'
                  ? 'The cards have begun to speak'
                  : 'Les cartes ont commencé à parler'}
              </h2>
              <p className="mt-4 max-w-2xl mx-auto text-violet-200 leading-8">
                {lang === 'en'
                  ? 'Each card reveals a fragment. The deeper meaning appears in the way they answer one another.'
                  : 'Chaque carte révèle un fragment. Le sens le plus profond apparaît dans la manière dont elles se répondent.'}
              </p>
            </div>

            <div className="mt-8 space-y-4">
              {cards.map((c, i) => (
                <article key={`${c.name}-${i}`} className="rounded-2xl bg-white/[0.055] border border-white/10 p-5 md:p-6">
                  <div className="text-xs uppercase tracking-[0.18em] text-amber-100/65">
                    {labels[i]}
                  </div>
                  <h3 className="font-serif text-xl mt-1">{c.name}</h3>
                  <p className="mt-3 leading-8 text-violet-100">
                    {freeCardReading(c, lang)}
                  </p>
                </article>
              ))}
            </div>
            ...
            <div className="mt-8 rounded-[28px] border border-amber-100/25 bg-gradient-to-b from-amber-100/[0.09] to-white/[0.03] p-7 md:p-9 text-center shadow-2xl">
              ...
              <h2 className="text-2xl md:text-4xl font-serif mt-3">
                {lang === 'en'
                  ? 'Your spread holds more than five separate messages'
                  : 'Votre tirage contient bien plus que des cartes isolées'}
              </h2>

              <p className="mt-4 max-w-2xl mx-auto text-violet-100 leading-8">
                {lang === 'en'
                  ? 'The full reading reveals what links the cards together, what remains hidden, where tensions appear and what direction the spread may be opening.'
                  : 'La lecture complète révèle ce qui relie les cartes entre elles, ce qui reste encore dans l’ombre, où se situent les tensions et quelle direction le tirage semble ouvrir.'}
              </p>
              ...
              <div className="grid md:grid-cols-3 gap-3 mt-7 text-left">
                {[
                  ...
                  lang === 'en' ? 'What your question awakens' : 'Ce que votre question met en mouvement',
                  lang === 'en' ? 'The deeper symbolic message' : 'Le message symbolique profond'
                ].map(label => (
                  <div key={label} className="rounded-2xl border border-white/10 bg-black/15 p-5">
                    <div className="text-xl text-amber-100/70">✦</div>
                    <div className="mt-3 text-violet-100">{label}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-8 rounded-[28px] bg-white/[0.045] border border-white/10 p-6 md:p-8">
              <div className="text-center">
                <div className="text-xs uppercase tracking-[0.22em] text-violet-300">
                  {lang === 'en' ? 'And if a question remains…' : 'Et si une question demeure…'}
                </div>
                <h3 className="text-2xl md:text-3xl font-serif mt-3">
                  {lang === 'en' ? 'Continue privately with Nanou' : 'Poursuivez en privé avec Nanou'}
                </h3>
                <p className="mt-3 text-violet-200 leading-7 max-w-2xl mx-auto">
                  {lang === 'en'
                    ? 'After the Premium reading, you can continue the exchange directly with Nanou in the secure private chat.'
                    : 'Après la lecture Premium, vous pourrez prolonger l’échange directement avec Nanou dans le chat privé sécurisé.'}
                </p>
              </div>
              <div className="mt-6">
                <LiveChatPromo />
      ...
      <style jsx global>{`
        @keyframes cardReveal {
          0% { opacity: 0; transform: translateY(-20px) rotateY(90deg) scale(.94); }
          55% { opacity: 1; transform: translateY(4px) rotateY(12deg) scale(1.02); }
