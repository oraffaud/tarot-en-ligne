export default async function handler(req, res) {
  ...
  const labels = spreadSize === 1
    ...
    : spreadSize === 3
      ...
      : (lang === 'en'
          ? ['Context', 'Challenge', 'Guidance', 'Influences', 'Possible outcome']
          : ['Contexte', 'Défi', 'Éclairage', 'Influences', 'Issue possible'])

  const userPrompt = `
Question: ${question || (lang === 'en' ? '(no question provided)' : '(aucune question précisée)')}
  ...
  "cards": [
    {
      ...
      "reading": "2-4 flowing sentences explaining what this card may illuminate here",
      "shadow": "one gentle sentence about the tension or point of attention, without alarmism",
      "invitation": "one intimate reflective invitation or question"
    }
  ],
...
Do not include bullet-style business language such as opportunities, cautions, action plan, KPI, synthesis, themes or tone. Do not repeat card dictionary meanings mechanically.`
  ...
  function demoPayload() {
    const demoCards = cards.slice(0, spreadSize).map((c, i) => ({
      ...
      reading: lang === 'en'
        ? 'This card invites you to listen to what is quietly taking shape beneath the surface. Its energy suggests a movement that deserves to be welcomed rather than rushed.'
        : 'Cette carte vous invite à écouter ce qui cherche doucement à prendre forme sous la surface. Son énergie suggère un mouvement qu’il vaut mieux accueillir que précipiter.',
      shadow: lang === 'en'
        ? 'The point of attention is to avoid forcing an answer before it feels ripe.'
        : 'Le point d’attention est de ne pas forcer une réponse avant qu’elle ne soit mûre.',
      invitation: lang === 'en'
        ? 'What becomes clearer when you stop trying to control the outcome?'
        : 'Qu’est-ce qui devient plus clair lorsque vous cessez de vouloir contrôler l’issue ?'
    }))

    return {
      title: lang === 'en' ? 'A path that asks to be listened to' : 'Un chemin qui demande à être écouté',
      opening: lang === 'en'
        ? 'There is a quiet but meaningful movement in this spread. The cards do not impose an answer; they invite you to notice what is already resonating within you.'
        : 'Il se dégage de ce tirage un mouvement discret mais profond. Les cartes n’imposent pas une réponse : elles vous invitent plutôt à reconnaître ce qui résonne déjà en vous.',
      cards: demoCards,
      connections: lang === 'en'
        ? 'Together, the cards describe a progression from awareness toward a more conscious choice. What first appears uncertain may become clearer once the different energies are allowed to speak to one another.'
        : 'Ensemble, les cartes racontent un passage de la prise de conscience vers un choix plus aligné. Ce qui semble d’abord incertain peut s’éclairer lorsque les différentes énergies du tirage sont mises en dialogue.',
      message: lang === 'en'
        ? 'The central invitation is to trust clarity that comes from within rather than from urgency. Your freedom of choice remains at the heart of the reading.'
        : 'L’invitation centrale est de faire confiance à une clarté qui vient de l’intérieur plutôt qu’à l’urgence. Votre liberté de choix reste au cœur du tirage.',
      nanou_question: lang === 'en'
        ? 'If you had nothing to prove, what would you choose to listen to today?'
        : 'Si vous n’aviez rien à prouver, qu’auriez-vous envie d’écouter aujourd’hui ?',
      closing: lang === 'en'
        ? 'Keep what resonates and leave the rest. Tarot is a mirror, not a sentence.'
        : 'Gardez ce qui résonne et laissez le reste. Le tarot est un miroir, jamais une sentence.'
    }
  }
  ...
  async function compute() {
    ...
    if (MODE === 'live') {
      ...
      return { parsed, meta: { mode: 'live' } }
    }

    if (!apiKey) {
      return { parsed: demoPayload(), meta: { mode: 'demo', reason: 'no_api_key' } }
    }
