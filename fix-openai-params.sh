...
export default async function handler(req, res) {
  ...
  // ---- Responses API (avec text.format au lieu de response_format) ----
  async function callResponses() {
    const r = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        // On demande du texte formaté en JSON objet
        modalities: ['text'],
  ...
  // ---- Fallback Chat API (attention: 429 si quota insuffisant) ----
  ...
  // Logging local/dev uniquement (FS éphémère en prod Vercel)
...
echo "✅ Paramétrage OpenAI corrigé (Responses API: text.format)."
